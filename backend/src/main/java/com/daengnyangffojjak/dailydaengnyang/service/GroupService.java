...
public class GroupService {
    ...
    public Group getGroupById(Long groupId){    //그룹 아이디로 그룹 조회, 없으면 예외 발생
        return groupRepository.findById(groupId).orElseThrow(() -> new GroupException(ErrorCode.GROUP_NOT_FOUND));
    }
    public List<UserGroup> getUserGroupListWithUsername(Long groupId, String username){ //유저가 그룹 내 멤버이면 그룹유저리스트 반환
        User user = userRepository.findByUserName(username)
                .orElseThrow(() -> new UserException(ErrorCode.USERNAME_NOT_FOUND));     //유저 확인
        //그룹이 존재하는지 확인
