...
public class ScheduleRestController {
    ...
    public ResponseEntity<Response<ScheduleCreateResponse>> createSchedule(@PathVariable Long petId, @RequestBody ScheduleCreateRequest scheduleCreateRequest, @AuthenticationPrincipal UserDetails user){
        ...
        return ResponseEntity.created(URI.create("api/v1/pets/"+petId+"/schedules"+scheduleCreateResponse.getId()))
    ...
    public ResponseEntity<Response<ScheduleDeleteResponse>> deleteSchedule(@PathVariable Long petId, @PathVariable Long scheduleId, @AuthenticationPrincipal UserDetails user){
        ScheduleDeleteResponse scheduleDeleteResponse = scheduleService.delete(petId, scheduleId, user.getUsername());
        return ResponseEntity.ok().body(Response.success(scheduleDeleteResponse));
    }

    // 일정 상세 조회(단건)
    @GetMapping(value = "/pets/{petId}/schedules/{scheduleId}")
    public ResponseEntity<Response<ScheduleResponse>> getSchedule(@PathVariable Long petId, @PathVariable Long scheduleId, @AuthenticationPrincipal UserDetails user){
        ScheduleResponse scheduleResponse = scheduleService.get(petId, scheduleId, user.getUsername());
